...
must_haves:
  ...
  key_links:
    ...
    - from: "scripts/test-subscription-harness.ts"
      to: "ws (WebSocket)"
      via: "BinanceAdapter or raw WebSocket connection for 2s streaming test"
...
1. Connect to the Binance WebSocket using the exchange's `wsUrl` from the DB
2. Use a raw WebSocket connection (not full BinanceAdapter) to keep the test lightweight:
   - Connect to `{wsUrl}/ws`
   - Send a SUBSCRIBE message for `btcusdt@kline_1m` stream
   - Listen for 2 seconds
   - Count received messages, parse at least one kline message to verify data shape
3. Disconnect after 2 seconds
4. Mark test as PASS if at least 1 kline message was received and successfully parsed (has `k.o`, `k.c`, `k.h`, `k.l`, `k.v` fields)
