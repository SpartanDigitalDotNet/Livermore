...
<task type="auto">
  ...
  <files>packages/schemas/src/command/command.schema.ts</files>
  <action>
Check packages/schemas for CommandTypeSchema and add 'bulk-add-symbols' if not present.

Look for the command schema file and update the CommandType enum/union to include 'bulk-add-symbols'.
...
If the schema file is in a different location, find it first:
`ls packages/schemas/src/**/*.ts | grep -i command`
  </action>
  <verify>
