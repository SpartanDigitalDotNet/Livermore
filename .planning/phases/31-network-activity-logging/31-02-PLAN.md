...
<task type="auto">
  ...
  <action>
    ...
    **Part B: Create and wire NetworkActivityLogger in server.ts**
    ...
    2. After creating `instanceRegistry` and `stateMachine` (around line 255-260), create the activity logger. First add import at top of file: `import { hostname } from 'node:os';`
       Then create:
       ```typescript
    ...
    4. After creating the activityLogger, start an async IP update. Add import at top of file: `import { detectPublicIp } from './utils/detect-public-ip';`
       Then after creating the activityLogger:
       ```typescript
    ...
    7. In `control-channel.service.ts`, add static imports at the top:
       ```typescript
       ...
       Then in `handleStart()` (around line 508-514), after creating the new registry but BEFORE creating state machine, change the block to:
       ```typescript
