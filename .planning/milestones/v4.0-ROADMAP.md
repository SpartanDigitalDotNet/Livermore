...
**Issues Resolved:**
- Bidirectional form/JSON sync with infinite loop prevention
- Vite HMR compatibility for hooks (useSettingsForm extracted to own module)
- Real-time WebSocket alerts with MACD-V colored UI elements

**Issues Deferred:**
- Router auth hardening (indicator, alert, position routers) — v4.1
- Full switch-mode strategy implementation — v4.1
- control.getStatus mock data — accepted limitation

**Technical Debt Incurred:**
- control.getStatus returns mock data (requires ControlChannelService in tRPC context)
